...
def create_step5_visualizations():
    ...
    create_category_sites_distribution(
        figures_path
    )  # Keep this - shows sites per category
...
def create_distance_distribution(high_risk_sites, figures_path):
    ...
    if "Distance_to_River_m" not in high_risk_sites.columns:
        print("No distance data found")
        return

    fig, ax = setup_professional_plot(figsize=(10, 6))

    distances = high_risk_sites["Distance_to_River_m"]
    ...
    ax.axvline(
        median_dist, color=COLORS["neutral"], linestyle="--", linewidth=2, alpha=0.8
    )

    # Clean labels
    ax.set_xlabel("Distance to River (meters)", fontsize=12)
    ax.set_ylabel(
        "Number of Sites", fontsize=12
    )  # This is correct - one row per site in general assessment
...
def create_activity_distribution(high_risk_sites, figures_path):
    ...
    for activities_str in high_risk_sites["Lokalitetensaktivitet"].dropna():
        activities = [
            act.strip() for act in str(activities_str).split(";") if act.strip()
        ]
    ...
    colors = [
        COLORS["primary"] if i % 2 == 0 else COLORS["accent"]
    ...
    for i, (bar, count) in enumerate(zip(bars, activity_counts.values)):
        ax.text(
            count + max(activity_counts) * 0.01,
            i,
            f"{count}",
            va="center",
            fontweight="bold",
    ...
    ax.text(
        0.02,
        0.98,
        f"Total: {total_activities:,} aktivitetsforekomster",
        transform=ax.transAxes,
        va="top",
        ha="left",
...
def create_industry_distribution(high_risk_sites, figures_path):
    ...
    for industries_str in high_risk_sites["Lokalitetensbranche"].dropna():
        industries = [
            ind.strip() for ind in str(industries_str).split(";") if ind.strip()
        ]
    ...
    colors = [
        COLORS["secondary"] if i % 2 == 0 else COLORS["danger"]
    ...
    for i, (bar, count) in enumerate(zip(bars, industry_counts.values)):
        ax.text(
            count + max(industry_counts) * 0.01,
            i,
            f"{count}",
            va="center",
            fontweight="bold",
    ...
    ax.text(
        0.02,
        0.98,
        f"Total: {total_industries:,} brancheforekomster",
        transform=ax.transAxes,
        va="top",
        ha="left",
        bbox=dict(boxstyle="round,pad=0.3", facecolor=COLORS["light_gray"], alpha=0.8),
...
def create_substance_distribution(high_risk_sites, figures_path):
    ...
    ax.set_title(
        "Top 15 Substances (General Assessment - 500m threshold)", fontsize=14, pad=15
    )
...
def create_compound_boxplot_comparison(figures_path):
    ...
    try:
        ...
        unique_site_counts = [
            len(category_unique_sites.get(cat, set())) for cat in categories
        ]

        # Create labels with both occurrence and site counts
        labels_with_counts = [
            f"{cat}\n({sites} sites, {occur} occur.)"
            for cat, sites, occur in zip(
                categories, unique_site_counts, category_counts
            )
        ]
        ...
        for i, (patch, category, count) in enumerate(
            zip(box_plot["boxes"], categories, category_counts)
        ):
            if category == "ANDRE":
        ...
        ax.set_title(
            "Distance Distribution by Compound Category (Colored by Occurrence Count)",
            fontsize=14,
            pad=15,
        )
        ...
        total_unique_sites = (
            len(set().union(*category_unique_sites.values()))
            if category_unique_sites
            else 0
        )
...
def create_category_sites_distribution(figures_path):
    ...
    try:
        ...
        sorted_categories = sorted(
            category_sites.items(), key=lambda x: x[1], reverse=True
        )
        ...
        for cat in categories:
            if cat == "ANDRE":
                ...
                colors1.append(
                    COLORS["categories"][len(colors1) % len(COLORS["categories"])]
                )
        ...
        for i, (bar, count) in enumerate(zip(bars1, site_counts)):
            ax1.text(
                ...
                bar.get_height() + max(site_counts) * 0.01,
                f"{count}",
                ha="center",
                va="bottom",
                fontweight="bold",
        ...
        ax1.set_title(
            "Unique Sites Affected by Each Category", fontsize=14, fontweight="bold"
        )
        ...
        for bar, count in zip(bars2a, site_counts):
            if count > 0:
                ax2.text(
                    bar.get_x() + bar.get_width() / 2,
                    bar.get_height() + max(occurrence_counts) * 0.01,
                    f"{count}",
                    ha="center",
                    va="bottom",
                    fontsize=9,
        ...
        ax2.set_title(
            "Sites vs Occurrences by Category", fontsize=14, fontweight="bold"
        )
        ax2.set_xticks(x)
        ...
        avg_substances_per_site = (
            total_occurrences / total_sites if total_sites > 0 else 0
        )
...
â€¢ Categories: {len(categories)}"""
        ...
        plt.suptitle(
            "Category Impact Analysis: Sites and Occurrences",
            fontsize=16,
            fontweight="bold",
            y=1.02,
        )
...
def create_compound_distance_histogram(category_data, category, figures_path):
    ...
    distances = category_data["Distance_to_River_m"].dropna()
    if len(distances) == 0:
        return
    ...
    ax.hist(
        distances, bins=30, color=color, alpha=0.7, edgecolor="white", linewidth=0.5
    )

    ax.set_xlabel("Distance to River (meters)", fontsize=12)
    ax.set_ylabel("Number of Occurrences", fontsize=12)
    ax.set_title(
        f"{category}: Distance Distribution ({unique_sites:,} unique sites, {total_occurrences:,} occurrences)",
        fontsize=14,
    )
...
def create_compound_activity_distribution(category_data, category, figures_path):
    ...
    for activity_string in activities:
        individual_activities = [
            act.strip() for act in str(activity_string).split(";") if act.strip()
        ]
    ...
    for i, (bar, count) in enumerate(zip(bars, top_activities.values())):
        ax.text(
            bar.get_width() + max(top_activities.values()) * 0.01,
            bar.get_y() + bar.get_height() / 2,
            f"{count}",
            va="center",
            ha="left",
    ...
    ax.set_title(
        f"{category}: Top Aktiviteter ({unique_sites:,} sites, {unique_activities:,} unikke aktiviteter)",
        fontsize=14,
    )
    ax.grid(True, alpha=0.3, axis="x")
    ax.invert_yaxis()
...
def create_compound_industry_distribution(category_data, category, figures_path):
    ...
    for industry_string in industries:
        individual_industries = [
            ind.strip() for ind in str(industry_string).split(";") if ind.strip()
        ]
    ...
    for i, (bar, count) in enumerate(zip(bars, top_industries.values())):
        ax.text(
            bar.get_width() + max(top_industries.values()) * 0.01,
            bar.get_y() + bar.get_height() / 2,
            f"{count}",
            va="center",
            ha="left",
    ...
    ax.set_title(
        f"{category}: Top Brancher ({unique_sites:,} sites, {unique_industries:,} unikke brancher)",
        fontsize=14,
    )
    ax.grid(True, alpha=0.3, axis="x")
    ax.invert_yaxis()
