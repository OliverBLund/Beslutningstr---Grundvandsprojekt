...
if __package__ is None or __package__ == "":
    sys.path.append(str(Path(__file__).resolve().parents[2]))  # Go up 2 levels to Kode/
...
INPUT_FILES = {
    # Step 5 outputs
    "step5_compound_detailed": get_output_path(
        "step5_compound_detailed_combinations"
    ),  # Core scenario
    "step5_parked_sites": get_output_path(
        "step5_unknown_substance_sites"
    ),  # Parked sites
    ...
    "denmark_regions_shapefile": DATA_DIR
    / "regionsinddeling",  # Optional backdrop for maps
}
...
def load_gvfk_area_volume():
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            gvfk_data[gvfk_name] = {"area_km2": area, "volume_m³": volume}
...
def load_substance_sites():
    ...
    if not os.path.exists(detailed_path):
        raise FileNotFoundError(
            f"Compound-specific detailed combinations not found: {detailed_path}"
        )
    ...
    if (
        "GVFK" in detailed_combinations.columns
        and "Closest_GVFK" not in detailed_combinations.columns
    ):
        detailed_combinations = detailed_combinations.rename(
            columns={"GVFK": "Closest_GVFK"}
        )

    if (
        "Distance_to_River_m" in detailed_combinations.columns
        and "Final_Distance_m" not in detailed_combinations.columns
    ):
        detailed_combinations = detailed_combinations.rename(
    ...
    substance_sites = detailed_combinations.drop_duplicates(
...
def load_and_filter_branch_sites():
    ...
    if (
        "Distance_to_River_m" in unknown_sites.columns
        and "Final_Distance_m" not in unknown_sites.columns
    ):
        unknown_sites = unknown_sites.rename(
            columns={"Distance_to_River_m": "Final_Distance_m"}
        )
...
def run_phase1():
    ...
    sites_in_new_gvfks = filter_sites_by_gvfk_category(
        branch_sites, gvfk_categories["new_gvfks"]
    )
...
def calculate_gvfk_metrics(gvfk_set, gvfk_area_volume):
    ...
    return {"count": count, "area_km2": total_area, "volume_m³": total_volume}
...
def create_gvfk_count_progression_chart(shapefiles, gvfk_categories, output_dir):
    ...
    ax.set_title(
        "GVFK Progression Gennem Workflow", fontweight="bold", fontsize=20, pad=15
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "gvfk_count_progression.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_area_volume_progression_chart(
    shapefiles, gvfk_categories, gvfk_area_volume, output_dir
):
    ...
    ax1.bar(
        ...
        edgecolor="black",
        linewidth=0.6,
        label="Areal (km²) – kerne del",
    )
    ax1.bar(
        x - width / 2,
        area_branch_arr,
        width,
        bottom=area_core_arr,
        color="#FFC107",
        alpha=0.8,
        edgecolor="black",
        linewidth=0.6,
    ...
    ax2.bar(
        ...
        edgecolor="black",
        linewidth=0.6,
        label="Volumen (m³) – kerne del",
    )
    ax2.bar(
        x + width / 2,
        volume_branch_arr,
        width,
        bottom=volume_core_arr,
        color="#FB8C00",
        alpha=0.8,
        edgecolor="black",
        linewidth=0.6,
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "gvfk_area_volume_progression.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    if baseline_area and baseline_area > 0:
        ...
        print(
            f"    Step 5b (core): {core_area:,.0f} km² ({core_area / baseline_area * 100:.1f}% of baseline area)"
        )
        print(
            f"    Step 5b⁺ (expanded): {expanded_area:,.0f} km² ({expanded_area / baseline_area * 100:.1f}% of baseline area)"
        )
        if baseline_volume and baseline_volume > 0:
            print(
                f"    Step 5b (core): {core_volume:,.2e} m³ ({core_volume / baseline_volume * 100:.1f}% of baseline volume)"
            )
            print(
                f"    Step 5b⁺ (expanded): {expanded_volume:,.2e} m³ ({expanded_volume / baseline_volume * 100:.1f}% of baseline volume)"
            )
        if core_area > 0:
            print(
                f"    Additional area from branch-only GVFKs: {area_branch_arr[-1]:,.0f} km² (+{area_branch_arr[-1] / core_area * 100:.1f}% vs core)"
            )
        if core_volume > 0:
            print(
                f"    Additional volume from branch-only GVFKs: {volume_branch_arr[-1]:.2e} m³ (+{volume_branch_arr[-1] / core_volume * 100:.1f}% vs core)"
            )
...
def create_progression_table(shapefiles, gvfk_categories, gvfk_area_volume, output_dir):
    ...
    core_metrics = calculate_gvfk_metrics(
        gvfk_categories["core_gvfks"], gvfk_area_volume
    )
    rows.append(
        {
    ...
    expanded_metrics = calculate_gvfk_metrics(
        gvfk_categories["expanded_gvfks"], gvfk_area_volume
    )
    rows.append(
        {
    ...
    if baseline_count > 0:
        ...
        df["% of Baseline Volume"] = (df["Volume (m³)"] / baseline_volume * 100).round(
            1
        )
...
def create_expansion_breakdown_chart(gvfk_categories, gvfk_area_volume, output_dir):
    ...
    bars1 = ax1.bar(
        categories, counts, color=colors_plot, edgecolor="black", linewidth=0.5
    )
    ...
    bars2 = ax2.bar(
        categories, areas, color=colors_plot, edgecolor="black", linewidth=0.5
    )
    ...
    bars3 = ax3.bar(
        categories, volumes, color=colors_plot, edgecolor="black", linewidth=0.5
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "gvfk_expansion_breakdown.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_core_branch_area_summary(
    shapefiles,
    gvfk_categories,
    gvfk_area_volume,
    substance_sites,
    ...
    if "all_dk" in shapefiles:
        baseline_metrics = calculate_gvfk_metrics(
            set(shapefiles["all_dk"]["Navn"].unique()), gvfk_area_volume
        )
    ...
    rows = [
        ...
        },
        {
            ...
            "Area (km²)": core_metrics["area_km2"],
            "Volume (m³)": core_metrics["volume_m³"],
            "Avg sites/GVFK": round(len(substance_sites) / core_metrics["count"], 2)
            if core_metrics["count"]
            else None,
        },
        {
            ...
            "Avg sites/GVFK": round(len(new_gvfk_sites) / new_metrics["count"], 2)
            if new_metrics["count"]
            else None,
        },
        {
            ...
            "Area (km²)": expanded_metrics["area_km2"],
            "Volume (m³)": expanded_metrics["volume_m³"],
            "Avg sites/GVFK": round(expanded_site_count / expanded_metrics["count"], 2)
            if expanded_metrics["count"]
            else None,
        },
    ]
    ...
    if baseline_metrics and baseline_metrics["area_km2"]:
        summary_df["% of Baseline Area"] = (
            summary_df["Area (km²)"] / baseline_metrics["area_km2"] * 100
        ).round(1)
    if baseline_metrics and baseline_metrics["volume_m³"]:
        summary_df["% of Baseline Volume"] = (
            summary_df["Volume (m³)"] / baseline_metrics["volume_m³"] * 100
        ).round(1)
    if baseline_metrics and baseline_metrics["count"]:
        summary_df["% of Baseline GVFKs"] = (
            summary_df["GVFK Count"] / baseline_metrics["count"] * 100
        ).round(1)

    summary_df["% of Expanded GVFKs"] = (
        summary_df["GVFK Count"] / expanded_metrics["count"] * 100
    ).round(1)
    summary_df["% of Expanded Area"] = (
        summary_df["Area (km²)"] / expanded_metrics["area_km2"] * 100
    ).round(1)
    summary_df["% of Expanded Volume"] = (
        summary_df["Volume (m³)"] / expanded_metrics["volume_m³"] * 100
    ).round(1)
...
def run_phase2(data, output_dir):
    ...
    create_area_volume_progression_chart(
        shapefiles, gvfk_categories, gvfk_area_volume, output_dir
    )
    progression_df = create_progression_table(
        shapefiles, gvfk_categories, gvfk_area_volume, output_dir
    )
    create_expansion_breakdown_chart(gvfk_categories, gvfk_area_volume, output_dir)
    create_core_branch_area_summary(
        shapefiles,
        gvfk_categories,
        gvfk_area_volume,
        data["substance_sites"],
        data["branch_sites"],
        data["sites_in_new_gvfks"],
        output_dir,
    )
...
def analyze_three_groups(substance_sites, all_branch_sites, new_gvfk_sites):
    ...
    print(
        f"    Branches: {len(branches_A)} unique, {branches_A.sum():,} total occurrences"
    )
    print(
        f"    Activities: {len(activities_A)} unique, {activities_A.sum():,} total occurrences"
    )

    print(f"\n  Group B (All branch-only): {len(all_branch_sites):,} sites")
    print(
        f"    Branches: {len(branches_B)} unique, {branches_B.sum():,} total occurrences"
    )
    print(
        f"    Activities: {len(activities_B)} unique, {activities_B.sum():,} total occurrences"
    )

    print(f"\n  Group C (New GVFK sites only): {len(new_gvfk_sites):,} sites")
    print(
        f"    Branches: {len(branches_C)} unique, {branches_C.sum():,} total occurrences"
    )
    print(
        f"    Activities: {len(activities_C)} unique, {activities_C.sum():,} total occurrences"
    )

    # Overlap analysis
    branch_overlap = analyze_overlap(branches_A, branches_B, branches_C, "Branches")
    activity_overlap = analyze_overlap(
        activities_A, activities_B, activities_C, "Activities"
    )
...
def create_three_way_comparison_chart(comparison_data, data_type, output_dir):
    ...
    # Truncate long names
    item_names = [
        name[:40] + "..." if len(name) > 40 else name for name in top_15_items
    ]
    ...
    ax.set_title(
        f"Top 15 {data_type_da}: Tre-vejs Sammenligning",
        fontweight="bold",
        fontsize=20,
        pad=15,
    )
    ...
    for bar, count in zip(bars3, counts_C):
        if count > 0:
            ...
            ax.text(
                width_val + max(counts_A + counts_B + counts_C) * 0.01,
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"three_way_{data_type}_comparison.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_two_way_comparison_chart(series_A, series_B, data_type, output_dir):
    ...
    # Truncate long names
    item_names = [
        name[:50] + "..." if len(name) > 50 else name for name in top_20_items
    ]
    ...
    data_type_da = "Brancher" if data_type == "branches" else "Aktiviteter"
    ax1.set_title(
        f"Top 20 {data_type_da}: Absolutte Antal",
        fontweight="bold",
        fontsize=20,
        pad=15,
    )
    ...
    data_type_da = "Brancher" if data_type == "branches" else "Aktiviteter"
    ax2.set_title(
        f"Top 20 {data_type_da}: Relativ Fordeling",
        fontweight="bold",
        fontsize=20,
        pad=15,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"two_way_{data_type}_comparison.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_overlap_visualization(branch_overlap, activity_overlap, output_dir):
    ...
    bars2 = ax2.bar(
        range(len(categories)), values_activity, color=colors_plot, alpha=0.8
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "overlap_analysis.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def create_comparison_tables(
    ...
    summary_data = {
        ...
        "Group A (Substance)": [
            ...
            branches_A.sum(),
            activities_A.sum(),
            f"{branches_A.index[0]} ({branches_A.iloc[0]})"
            if len(branches_A) > 0
            else "N/A",
            f"{activities_A.index[0]} ({activities_A.iloc[0]})"
            if len(activities_A) > 0
            else "N/A",
        ...
        "Group B (All Branch-only)": [
            ...
            branches_B.sum(),
            activities_B.sum(),
            f"{branches_B.index[0]} ({branches_B.iloc[0]})"
            if len(branches_B) > 0
            else "N/A",
            f"{activities_B.index[0]} ({activities_B.iloc[0]})"
            if len(activities_B) > 0
            else "N/A",
        ...
        "Group C (New GVFKs only)": [
            ...
            branches_C.sum(),
            activities_C.sum(),
            f"{branches_C.index[0]} ({branches_C.iloc[0]})"
            if len(branches_C) > 0
            else "N/A",
            f"{activities_C.index[0]} ({activities_C.iloc[0]})"
            if len(activities_C) > 0
            else "N/A",
    ...
    top_branches = create_top_15_comparison_table(
        branches_A, branches_B, branches_C, "branches", output_dir
    )

    # Table 3: Top 15 activities comparison
    top_activities = create_top_15_comparison_table(
        activities_A, activities_B, activities_C, "activities", output_dir
    )
...
def run_phase3(data, output_dir):
    ...
    comparison_results = analyze_three_groups(
        substance_sites, all_branch_sites, new_gvfk_sites
    )

    # Create visualizations
    create_three_way_comparison_chart(
        comparison_results["branches"], "branches", output_dir
    )
    create_three_way_comparison_chart(
        comparison_results["activities"], "activities", output_dir
    )

    # Create improved 2-way comparison charts (Group A vs Group B only)
    create_two_way_comparison_chart(
        comparison_results["branches"]["A"],
        comparison_results["branches"]["B"],
        "branches",
        output_dir,
    )
    create_two_way_comparison_chart(
        comparison_results["activities"]["A"],
    ...
    create_overlap_visualization(
        comparison_results["branch_overlap"],
...
def create_hexagonal_heatmap(sites_gdf, title, output_path, hex_size_km=10):
    ...
    hexbin = ax.hexbin(
        x,
        y,
        gridsize=int(100000 / hex_size_m),
        cmap="plasma",
        mincnt=1,
        alpha=0.8,
        edgecolors="gray",
    ...
    ax.text(
        0.02,
        0.98,
        f"Total lokaliteter: {len(sites_gdf):,}",
...
def create_side_by_side_heatmaps(
    substance_detailed, substance_sites, expanded_sites, branch_sites, output_dir
):
    """
    ...
    if not os.path.exists(step4_sites_path):
        print(
            f"  Warning: Step 4 unique lokalitet shapefile not found, skipping heatmaps"
        )
        return
    ...
    if "Lokalitets" in step4_gdf.columns:
        ...
        print(
            f"  Warning: Cannot find ID column. Available: {list(step4_gdf.columns)[:10]}"
        )
        return
    ...
    sm.set_array([])
    cbar = fig.colorbar(
        sm, ax=[ax1, ax2], shrink=0.8, aspect=30, pad=0.03, fraction=0.046
    )
...
def create_gvfk_choropleth_maps(
    shapefiles,
    substance_detailed,
    substance_sites,
    all_sites,
    ...
    expanded_detailed = pd.concat(
        [
            ...
            branch_sites[["Closest_GVFK", "Lokalitet_ID"]]
            if "Closest_GVFK" in branch_sites.columns
            else pd.DataFrame(),
        ],
        ignore_index=True,
    )
    expanded_counts = expanded_detailed.groupby("Closest_GVFK")[
        "Lokalitet_ID"
    ].nunique()
    ...
    ax1.set_title(
        f"Kerne Scenarie: {core_gvfk_count} GVFKs med Substanslokalteter\n({core_unique_sites:,} lokaliteter total)",
        fontsize=24,
        fontweight="bold",
        pad=15,
    )
...
def create_regional_distribution_table(
    substance_sites, all_sites, gvfk_categories, output_dir
):
    """
...
def create_new_gvfk_characteristics_table(
    new_gvfk_sites, gvfk_area_volume, gvfk_categories, output_dir
):
    """
    ...
    for gvfk in new_gvfks:
        ...
        top_branch = (
            f"{branches.index[0]} ({branches.iloc[0]})" if len(branches) > 0 else "N/A"
        )
        top_activity = (
...
def run_phase4(data, output_dir):
    ...
    create_side_by_side_heatmaps(
        substance_detailed, substance_sites, all_sites, branch_sites, output_dir
    )

    # Create choropleth maps
    create_gvfk_choropleth_maps(
        shapefiles,
        substance_detailed,
        substance_sites,
        all_sites,
    ...
    # Create tables
    regional_df = create_regional_distribution_table(
        substance_sites, all_sites, gvfk_categories, output_dir
    )

    new_gvfk_df = create_new_gvfk_characteristics_table(
        new_gvfk_sites, gvfk_area_volume, gvfk_categories, output_dir
    )
...
def compare_core_vs_new_gvfks(gvfk_categories, gvfk_area_volume, output_dir):
    """
    Compare characteristics of Core GVFKs vs New GVFKs.
    ...
    if os.path.exists(step3_path):
        ...
        if gvfk_col and site_id_col:
            ...
            print(
                f"  Loaded V1/V2 site data: {total_unique_sites:,} unique sites ({len(step3_gdf):,} total rows with duplicates)"
            )

            # Store total unique sites in each category (for summary table later)
            core_v1v2_counts["_total_unique"] = step3_gdf[
                step3_gdf[gvfk_col].isin(core_gvfks)
            ][site_id_col].nunique()
            new_v1v2_counts["_total_unique"] = step3_gdf[
                step3_gdf[gvfk_col].isin(new_gvfks)
            ][site_id_col].nunique()
        else:
            print(
                f"  Warning: Could not find GVFK column in Step 3 data. Available columns: {list(step3_gdf.columns)[:10]}"
            )
    else:
        print(
            f"  Warning: Step 3 data not found at {step3_path}, V1/V2 counts unavailable"
        )
    ...
    ax1 = axes[0, 0]
    bp1 = ax1.boxplot(
        [core_areas, new_areas],
        labels=["Kerne 240 GVFKs", "Nye 91 GVFKs"],
        patch_artist=True,
        showmeans=True,
    )
    ...
    if os.path.exists(core_detailed_path):
        ...
        if (
            "GVFK" in core_detailed_df.columns
            and "Closest_GVFK" not in core_detailed_df.columns
        ):
        ...
        if "Closest_GVFK" in core_detailed_df.columns:
            ...
            core_sites_per_gvfk = core_detailed_df.groupby("Closest_GVFK")[
                "Lokalitet_ID"
            ].nunique()
            for gvfk in core_gvfks:
    ...
    if os.path.exists(branch_sites_path):
        ...
        if (
            "Distance_to_River_m" in branch_df.columns
            and "Final_Distance_m" not in branch_df.columns
        ):
    ...
    if core_site_counts and new_site_counts:
        bp3 = ax3.boxplot(
            [core_site_counts, new_site_counts],
            labels=[
                "Kerne 240 GVFKs\n(1,743 total lokaliteter)",
                "Nye 91 GVFKs\n(~242 total lokaliteter)",
            ],
            patch_artist=True,
            showmeans=True,
        )
        ...
        ax3.set_ylabel(
            "Risikovurderingslokalteter per GVFK", fontsize=16, fontweight="bold"
        )
        ...
        ax3.text(
            0.5,
            0.5,
            "Lokalitetsdata ikke tilgængelig",
            ha="center",
            va="center",
            fontsize=16,
...
def run_step6_analysis():
    ...
    gvfk_comparison = compare_core_vs_new_gvfks(
        data["gvfk_categories"], data["gvfk_area_volume"], output_dir
    )
