...
def create_map(v1v2_with_distances, rivers_with_contact, valid_results, gvfk_polygons):
    ...
    gvfk_name_col = COLUMN_MAPPINGS['contamination_csv']['gvfk_id']  # "Navn"
    site_id_col = COLUMN_MAPPINGS['contamination_shp']['site_id']    # "Lokalitet_"
    ...
    sampled_gvfks = set(v1v2_web[gvfk_name_col].unique())
    relevant_gvfk_web = gvfk_web[gvfk_web[gvfk_name_col].isin(sampled_gvfks)]

    print(f"Adding {len(relevant_gvfk_web)} GVFK polygons for visualization")

    for idx, gvfk in relevant_gvfk_web.iterrows():
        gvfk_name = gvfk.get(gvfk_name_col, 'Unknown')
    ...
    unique_sites = v1v2_web.drop_duplicates(subset=[site_id_col])
    ...
    for idx, site in unique_sites.iterrows():
        ...
        site_data = v1v2_web[v1v2_web[site_id_col] == site_id]
        site_gvfks = sorted(list(set(site_data[gvfk_name_col].tolist())))  # Remove duplicates and sort
    ...
    for idx, result in all_distances_to_show.iterrows():
        ...
        site_combo = v1v2_web[
            (v1v2_web[site_id_col] == lokalitet_id) &
            (v1v2_web[gvfk_name_col] == gvfk)
        ]
