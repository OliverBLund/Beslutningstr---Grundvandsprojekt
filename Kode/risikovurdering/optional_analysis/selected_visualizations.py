...
def create_distance_histogram_with_thresholds(results_path):
    ...
    if os.path.exists(unique_distance_file):
        ...
        print(
            "Unique locations file not found. Using all geometries dataset for threshold visualization."
        )
    else:
        print("Required files for distance analysis not found.")
        print(
            "Run the step4_calculate_distances() function in workflow.py first to generate these files."
        )
        return
    ...
    try:
        ...
        print(
            f"Sites with distances > 20,000m: {distances_over_20k} ({distances_over_20k / len(distance_df) * 100:.1f}%)"
        )

        # Cap the distances at 20,000m
        distance_df["Distance_to_River_m"] = distance_df["Distance_to_River_m"].clip(
            upper=20000
        )
        ...
        if (
            analysis_type == "alle geometrier"
            and "Lokalitetsnr" in distance_df.columns
            and not distance_df["Lokalitetsnr"].isna().all()
        ):
            ...
            if unique_count > 0:
                ...
                unique_distance_df = distance_df.loc[
                    distance_df.groupby("Lokalitetsnr")["Distance"].idxmin()
                ]
                print(
                    f"After selecting minimum distance per location: {len(unique_distance_df)} records"
                )
        ...
        if "Distance_to_River_m" in distance_df.columns:
            ...
            ax2.yaxis.set_major_formatter(
                plt.FuncFormatter(lambda y, _: "{:.1%}".format(y / len(distance_df)))
            )
            ...
            title_suffix = (
                "unikke lokaliteter"
                if analysis_type == "unikke lokaliteter"
                else "V1/V2-lokaliteter"
            )
            plt.title(
                f"Afstande mellem {title_suffix} og nærmeste kontaktzone\nmed fremhævede tærskelværdier (afstande >20km grupperet)",
                fontsize=14,
                pad=20,
            )
            ...
            if "Site_Type" in distance_df.columns:
                ...
                site_type_elements = [
                    Patch(
                        facecolor=colors["v1"], label=f"V1 kun (n={len(v1_only_data)})"
                    ),
                    Patch(
                        facecolor=colors["v2"], label=f"V2 kun (n={len(v2_only_data)})"
                    ),
                    Patch(
                        facecolor=colors["v1v2"], label=f"V1 og V2 (n={len(both_data)})"
                    ),
                ]
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(figures_path, "distance_histogram_thresholds.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.savefig(
            ...
            for i, threshold in enumerate(thresholds):
                ...
                y_val = (distance_df["Distance_to_River_m"] <= threshold).sum() / len(
                    distance_df
                )
            ...
            plt.title(
                f"Kumulativ fordeling af afstande for {title_suffix}\nmed tærskelværdier",
                fontsize=14,
                pad=20,
            )
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(figures_path, "distance_cdf_thresholds.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.savefig(
