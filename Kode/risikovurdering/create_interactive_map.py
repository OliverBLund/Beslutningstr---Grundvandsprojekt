...
def create_map(v1v2_with_distances, rivers_with_contact, valid_results, gvfk_polygons):
    ...
    for _, gvfk in relevant_gvfk_web.iterrows():
    ...
    for _, site in unique_sites.iterrows():
        ...
        min_distance = (
            site_data["Min_Dist_m"].iloc[0]
            if "Min_Dist_m" in site_data.columns and not site_data["Min_Dist_m"].isna().all()
            else None
        )
        ...
        distance_range_text = (
            f"{site_distances[0]:.1f}m - {site_distances[-1]:.1f}m" if site_distances else "N/A"
        )
    ...
    for _, river in relevant_rivers.iterrows():
        ...
        folium.GeoJson(
            ...
            style_function=lambda x: {"color": "darkblue", "weight": 3, "opacity": 0.8},
            popup=folium.Popup(popup_text, max_width=200),
        ).add_to(m)
    ...
    print(f"- Non-minimum distances (lighter): {len(valid_results) - len(min_distance_results)}")

    all_distances_to_show = valid_results.copy()

    for _, result in all_distances_to_show.iterrows():
        ...
        site_combo = v1v2_web[
            (v1v2_web["Lokalitet_"] == lokalitet_id) & (v1v2_web["Navn"] == gvfk)
        ]
        if site_combo.empty:
            continue
        ...
        for _, river in matching_rivers.iterrows():
            ...
            if distance_calc < min_distance_calc:
                ...
                closest_point_on_site, closest_point_on_river = nearest_points(site_geom, river.geometry)

        if closest_point_on_site is None or closest_point_on_river is None:
            continue
        ...
        if is_minimum:
            ...
            label_style = (
                "font-size: 10pt; color: red; font-weight: bold; background-color: white; "
            ...
            label_style = (
                "font-size: 8pt; color: orange; background-color: white; padding: 1px; border: 1px solid orange;"
            )
        ...
        folium.PolyLine(
            ...
            popup=folium.Popup(
                f"{popup_prefix}<br>Lokalitet: {lokalitet_id}<br>GVFK: {gvfk}<br>Distance: {distance:.1f}m",
                max_width=300,
            ),
        ).add_to(m)
    ...
    print("Open the file in a web browser to explore the distance calculations!")
