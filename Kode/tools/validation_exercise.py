...
def print_step6_statistics(data):
    """Print Step 6 (Tilstandsvurdering) summary statistics."""
    print("\n" + "=" * 80)
    print("STEP 6 STATISTICS: TILSTANDSVURDERING SUMMARY")
    print("=" * 80)

    # Load Step 6 specific files
    flux_path = get_output_path('step6_flux_site_segment')
    cmix_path = get_output_path('step6_cmix_results')
    summary_path = get_output_path('step6_segment_summary')

    if not flux_path.exists() or not cmix_path.exists():
        print("  Step 6 output files not found")
        return

    flux = pd.read_csv(flux_path)
    cmix = pd.read_csv(cmix_path)
    seg_summary = pd.read_csv(summary_path) if summary_path.exists() else pd.DataFrame()

    # Overall impact
    print("\n  Overall Impact:")
    print(f"    Sites analyzed: {flux['Lokalitet_ID'].nunique():,}")
    print(f"    River segments: {flux['Nearest_River_FID'].nunique():,}")
    print(f"    GVFKs: {flux['GVFK'].nunique():,}")

    # MKK Exceedances
    if 'Exceedance_Flag' in cmix.columns:
        exc_cmix = cmix[cmix['Exceedance_Flag'] == True]

        print("\n  MKK Exceedances:")
        if not seg_summary.empty and 'Has_MKK_Exceedance' in seg_summary.columns:
            exc_segments = seg_summary[seg_summary['Has_MKK_Exceedance'] == True]
            print(f"    Segments with exceedance: {len(exc_segments):,}")

        if not exc_cmix.empty:
            print(f"    Exceedance records: {len(exc_cmix):,}")

            # By flow scenario
            print("\n  Exceedances by Flow Scenario:")
            for scenario in ['Q95', 'Q90', 'Q50', 'Q10', 'Q05']:
                scenario_exc = exc_cmix[exc_cmix['Flow_Scenario'] == scenario]
                if not scenario_exc.empty:
                    segments = scenario_exc['Nearest_River_FID'].nunique()
                    print(f"    {scenario}: {segments:,} segments")

            # Exceedance ratios
            if 'Exceedance_Ratio' in exc_cmix.columns:
                print("\n  Exceedance Ratios:")
                print(f"    Maximum: {exc_cmix['Exceedance_Ratio'].max():,.1f}x MKK")
                print(f"    Median: {exc_cmix['Exceedance_Ratio'].median():,.1f}x MKK")
        else:
            print("    No MKK exceedances found")
